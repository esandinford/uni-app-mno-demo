...
do {                                                                                 \
    [LogUtil appendLog:@"[%s][%s]%s[%d]:",LOG_TAG,level, __FUNCTION__, __LINE__];    \
    [LogUtil appendLog:format, ##__VA_ARGS__];                                       \
    [LogUtil appendLog:@"\n"];                                                       \
} while(0)
...
do {                                                                                        \
    [LogUtil appendLog:@"\033[31m [%s][%s]%s[%d]:",LOG_TAG, level, __FUNCTION__, __LINE__]; \
    [LogUtil appendLog:format, ##__VA_ARGS__];                                              \
    [LogUtil appendLog:@"\n"];                                                              \
} while(0)
